...
exports.add = function(req, res, next) {
  if (!req.body.article) return next(new Error('No articles'));
  var article = req.body.article;
  article.published = false;
  req.collections.article.insert(article, function(error, articleResponse) {
    if (error) return next(error);
...
exports.postArticle = function(req, res, next) {
  ...
  req.collections.articles.insert(article, function(error, articleResponse) {
    if (error) return next(error);
    res.render('post', {error: 'Article was added. Publish it onAdmin page.'});
  });
};
